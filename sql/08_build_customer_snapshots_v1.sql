-- Una fila por cliente y fecha de corte.
-- Las variables usan información disponible hasta el corte.
-- El futuro se utiliza únicamente para construir reorder_90d.

WITH snapshots AS (
    SELECT DATE '2025-08-31' AS snapshot_date, 'train' AS split
    UNION ALL SELECT DATE '2025-11-30', 'train'
    UNION ALL SELECT DATE '2026-03-01', 'validation'
    UNION ALL SELECT DATE '2026-06-01', 'test'
),
...
order_features AS (
    ...
    FROM history
    GROUP BY snapshot_date, split, user_id
),
...
labels AS (
    SELECT
        ...
        MIN(o.order_date) AS first_future_order_date
    FROM order_features AS f
    LEFT JOIN orders AS o
        ON f.user_id = o.user_id
        AND o.order_date > f.snapshot_date
        AND o.order_date <= DATE_ADD(f.snapshot_date, INTERVAL 90 DAY)
    GROUP BY f.snapshot_date, f.user_id
)
...
SELECT
    ...
    f.snapshot_date,
    f.split,
    DATE_ADD(f.snapshot_date, INTERVAL 90 DAY) AS label_end_date,
    ...
    SAFE_DIVIDE(
        COALESCE(i.ordered_value_365d, 0),
        f.orders_365d
    ) AS avg_order_value_365d,

    f.max_feature_order_date,
    i.max_feature_item_date,
    l.first_future_order_date,
    l.reorder_90d
